...
import  {Modal,Select,Input,DatePicker, Space,Cascader,Radio, notification } from 'antd'
...
const  Feesadd:FC<any> =(props):JSX.Element=> {
  ...
  const handleDetailData:any = (detailData: any, newDetailData: any)=> {
    for (let i in detailData) {
      ...
      if (detailData[i].children && detailData[i].children.length > 0) {
        newDetailData.push({
          value: detailData[i].id,// 这三个属性都是组件库级联组件所需要的属性
        ...
        // 子级不存在时，不用push items:[],否则级联选择器最后一页会出现空白
            newDetailData.push({ value: detailData[i].id, label: detailData[i].name });
          }
        }
  ...
  return (
    <div className='app'>
      <Modal width={460}  wrapClassName={'dialog'} title="新增缴费" open={props.showmodal} onOk={handleOk} onCancel={handleCancel}>
        <div className='addbox'>
          <div className='addform'>
            ...
            <div className='formitem'>
              <div>小区:</div>
              <div>
              <Cascader style={{ width: 240 }} options={buildingarr} onChange={onChange} placeholder="请选择" />
              </div>
            </div>
            <div className='formitem'>
              <div>缴费房间:</div>
              <div>
                <Radio.Group style={{ width: 240 }} onChange={radioChange} value={value}>
                  <Radio value={1}>全部</Radio>
                  <Radio onClick={gethousesid} value={2}>部分房间</Radio>
                </Radio.Group>
              </div>
            </div>
            <div className='formitem' >
              <div> </div>
              <div>
              <TextArea value={textareadefaultValue} style={{ width: 240 }} rows={3} disabled  />
              </div>
            </div>
          </div>
        </div>
      </Modal>
      <Parthouse id={adddata.buildingid} isModalOpen={isModalOpen} getid={gethouseids} isModalCloce={getChildrenValue}></Parthouse>
    </div>
  )
}
